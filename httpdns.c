...
static void httpRequest_init()
{
    ...
    if (httpdns.http_req == NULL)
    {
        ...
        encode_save  = https.encodeCode;
        https.encodeCode = 0;
        if (httpdns.tcpDNS_mode == 0)
        {
            memset(&tcp_http, 0, sizeof(tcp_t));
            tcp_http.reqType = HTTP;
            tcp_http.host = (char *)1;  //如果为NULL，modify_request函数可能会使其指向动态分配内存
            if (modify_request(strdup(HTTPDNS_REQUEST), strlen(HTTPDNS_REQUEST), &tcp_http) != 0)
                exit(1);
            httpdns.http_req = replace(tcp_http.ready_data, &tcp_http.ready_data_len, "[H]", 3, dest, strlen(dest));
            if (httpdns.http_req == NULL)
                error("out of memory.");
            httpdns.http_req_len = tcp_http.ready_data_len;
            memcpy(&httpdns.dst, &http.dst, sizeof(httpdns.dst));
        }
        /* 通过https模块构建CONNECT请求头 */
        if (global.tcp_listen_fd > -1 && \
            (global.mode == WAP_CONNECT || \
            (global.mode == NET_CONNECT && port != 80 && port != 8080) || httpdns.tcpDNS_mode == 1))
        {
            if (encode_save)
            {
                dataEncode(dest, strlen(dest), encode_save);
                httpdns.httpsProxy_encodeCode = encode_save;
            }
            httpdns.connect_request_len = default_ssl_request_len;
            copy_new_mem(default_ssl_request, default_ssl_request_len, &httpdns.connect_request);
            httpdns.connect_request = replace(httpdns.connect_request, &httpdns.connect_request_len, "[H]", 3, dest, strlen(dest));
            if (httpdns.connect_request == NULL)
                error("out of memory.");
            memcpy(&httpdns.dst, &https.dst, sizeof(httpdns.dst));
        }
        https.encodeCode = encode_save;
    }
    //如果设置http_req = "";则不创建请求头
    else if (httpdns.http_req_len > 0)
    {
